...
{
	// Token: 0x060084AD RID: 33965 RVA: 0x0036109F File Offset: 0x0035F29F
	[PublicizedFrom(EAccessModifier.Protected)]
	public void Start()
	{
		SkinnedMeshCombiner.CombineMeshes(base.transform);
	}

	// Token: 0x060084AE RID: 33966 RVA: 0x003610AC File Offset: 0x0035F2AC
	public static void CombineMeshes(Transform _transform)
	{
		...
		for (int j = 0; j < componentsInChildren.Length; j++)
		{
			...
			foreach (BoneWeight item in skinnedMeshRenderer2.sharedMesh.boneWeights)
			{
			...
			foreach (Transform item2 in skinnedMeshRenderer2.bones)
			{
				list4.Add(item2);
			}
		...
		skinnedMeshRenderer3.sharedMesh.CombineMeshes(list6.ToArray(), true, true);
