...
{
	// Token: 0x06005C47 RID: 23623 RVA: 0x00252970 File Offset: 0x00250B70
	public static IEnumerator CreateMaterials(XmlFile _xmlFile)
	{
		...
		using (IEnumerator<XElement> enumerator = root.Elements("material").GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				XElement xelement = enumerator.Current;
				string attribute = xelement.GetAttribute("id");
				if (attribute == null || attribute.Length == 0)
				{
					throw new Exception("Material defined without a tag 'id'");
				}
				MaterialBlock materialBlock = new MaterialBlock(attribute);
				foreach (XElement propertyNode in xelement.Elements())
				{
					materialBlock.Properties.Add(propertyNode, true, false);
				}
				...
				materialBlock.Properties.ParseFloat("explosionresistance", ref explosionResistance);
				materialBlock.ExplosionResistance = explosionResistance;
